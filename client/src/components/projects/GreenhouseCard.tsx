...
import { Home, ChevronRight, Wifi, WifiOff, Construction } from 'lucide-react';
...
export function GreenhouseCard({ greenhouse, projectKey }: GreenhouseCardProps) {
  ...
  const deviceStatus = greenhouse.deviceStatus;
  ...
  const statusText = !isReady
    ...
    : isOnline
      ? t('greenhouse.ready')
      : isUnknown
        ? t('greenhouse.checking')
        : t('greenhouse.offline');
...
export function GreenhouseCardSkeleton() {
  return (
    <Card>
      <div className="flex items-center gap-4">
        <div className="w-14 h-14 rounded-xl bg-gray-200 dark:bg-gray-700 animate-pulse flex-shrink-0" />
